    ...
    <PageInfo>
      ...
      <div className='flex flex-col items-center flex-grow w-full'>
        ...
        <div className='flex justify-between sm:w-full md:w-2/3 change'>
          <input type="text" className='flex-grow border-2 p-2 mb-5 sm:mb-0 nomarg' value={message} onChange={(e)=>{
            setMessage(e.target.value);
            console.log(message);
          }}/><button className='py-2 px-5 bg-slate-500 text-white sm:mb-0 mb-5 nomarg ' onClick={handleClick}>Send</button>
        </div>
      </div>
    </PageInfo>
  )
